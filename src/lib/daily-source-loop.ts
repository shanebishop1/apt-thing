...
export async function runDailySourceAgentLoop(
  ...
  const secondaryResult = runSecondaryZillowFixture({
    runId,
    identity,
    now,
    concurrencyLimit,
...
async function runStreetEasySource({
  ...
  const sourceFixture = liveMetadata.normalizedFixture ?? fixture;
  const skipped: DailyLoopSkippedCandidate[] = [];
...
async function maybeCollectStreetEasyLiveSafeMetadata({
  ...
  const baseUrl = (
    ...
    "https://api.realtyapi.com"
  ).replace(/\/$/, "");
  const detailMetadataByListing: Record<string, DailyLoopDetailRetryMetadata> = {};
  try {
    const searchResponse = await fetchImpl(`${baseUrl}/search/rent`, {
      method: "POST",
      headers: { "Content-Type": "application/json", Authorization: `Bearer ${apiKey}` },
      body: JSON.stringify(fixture.query),
    });
    ...
    const liveResults = normalizeStreetEasySearchPayload(searchPayload, fixture);
    const detailPayloads: Record<string, unknown> = {};
    for (const listing of liveResults) {
      const detailResponse = await fetchImpl(`${baseUrl}/rental_detailsbyid`, {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${apiKey}` },
        body: JSON.stringify({ listing_id: listing.listingId }),
      });
    ...
    const normalizedFixture = mergeStreetEasyDetails(liveResults, detailPayloads, fixture);
    const hasDetailFailures = Object.values(detailMetadataByListing).some(
      (metadata) => metadata.status !== "success",
    );
    return {
      liveAttempted: true,
      classification: normalizedFixture && !hasDetailFailures ? "success" : "partial",
      missingKey: false,
      queryAttempts: 1,
      detailMetadataByListing,
      failureCode: normalizedFixture
        ? hasDetailFailures
          ? "streeteasy-live-safe-detail-partial"
          : undefined
        : "streeteasy-live-safe-unrecognized-payload",
      normalizedFixture,
    };
  } catch {
    return {
...
function normalizeStreetEasySearchPayload(
  ...
  return records.flatMap((record, index) => {
    ...
    const listingId = stringField(record, ["listingId", "listing_id", "id"]);
    const url = stringField(record, ["sourceUrl", "url", "permalink", "listing_url"]);
...
function normalizeStreetEasyDetails(
  ...
  return {
    ...
    title: stringField(record, ["title", "name"]) ?? "StreetEasy listing",
    address:
      stringField(record, ["address", "display_address", "streetAddress"]) ?? "Unknown address",
    neighborhood: stringField(record, ["neighborhood", "area"]),
    borough: stringField(record, ["borough", "city"]) ?? "Manhattan",
    rent: numberField(record, ["rent", "price", "monthlyRent"]),
    bedrooms: numberField(record, ["bedrooms", "beds"]),
    bathrooms: numberField(record, ["bathrooms", "baths"]),
    availableAt: stringField(record, ["availableAt", "available_at", "availableDate"]),
    description: stringField(record, ["description", "details"]),
    amenities: stringArrayField(record, ["amenities"]),
    photos: stringArrayField(record, ["photos", "images"]),
